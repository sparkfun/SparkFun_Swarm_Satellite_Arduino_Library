...
void loop()
{
  ...
  if (count > 0)
  {
    ...
    uint64_t *ids = new uint64_t[count]; // Create storage for the message IDs
    mySwarm.listTxMessagesIDs(ids); // Ask the modem for a list of the IDs
  
    for (uint16_t i = 0; (i < count) && (i < 10); i++) // Print all the message IDs
    {
      Serial.print(i);
      Serial.print(F(" : "));
    ...
    Serial.println(F("D followed by the message number to delete that message. E.g. D0"));
    Serial.println(F("L followed by the message number to list that message. E.g. L1"));
    ...
    else if (c == 'L') // List a message
    {
      ...
      if ((c >= '0') && (c < ('0' + count)) && (c <= '9')) // Check for a valid number
      {
        // Allocate storage for the message. The message could be up to 2 * 192 bytes long. Include a null on the end.
        char *asciiHex = new char[385];
        uint32_t epoch;
        mySwarm.listTxMessage(ids[c - '0'], asciiHex, 385, &epoch); // List the message
        Serial.println();
        Serial.print(F("Message contents in ASCII Hex: "));
        Serial.println(asciiHex);
        ...
        delete[] asciiHex; // Free the storage
      }
    }
