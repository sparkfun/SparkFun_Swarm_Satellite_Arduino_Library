 ...
 *   Put the modem to sleep until a date/time using sleepMode
 * 
...
void setup()
{
  ...
  if (err != SWARM_M138_SUCCESS)
  {
    ...
    Serial.print(F("Freezing..."));
    while (1)
      ; // Do nothing more
  }

  // Add 1 minute to the time.
  // Do it manually just to show what is going on.
  // Note: this code does not increment the date. See notes below.
  dateTime.mm += 1; // Add 1 minute
  if (dateTime.mm >= 60) // Check for an hour rollover
  {
    dateTime.mm -= 60;
    dateTime.hh += 1;
  }
  if (dateTime.hh >= 24) // Check for a day rollover
  {
    dateTime.hh -= 24;
  }

  // Put the modem to sleep
  Serial.println(F("Putting the modem to sleep for 1 minute"));

  // Sleep until the specified time - and ignore the date
  // Note: sleepMode defaults to sleeping until a date and time.
  //       Setting the dateAndTime parameter to false makes it sleep until the specified time (only)
  err = mySwarm.sleepMode(dateTime, false);
