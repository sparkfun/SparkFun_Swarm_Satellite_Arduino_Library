...
void Predict(int many){
    ...
    for (int i = 0; i<many ; i++){
        ...
        if ( error == 1){ //no error, prints overpass information
          ...
          Serial.printf("%d-%02d-%02dT%02d:%02d:%s%.2fZ,", year, mon, day, hr, minute, sec < 10.0 ? "0" : "", sec);
          long startTime = (hr * 3600) + (minute * 60) + sec;
          
          invjday(overpass.jdstop ,timezone ,true , year, mon, day, hr, minute, sec);
          Serial.printf("%d-%02d-%02dT%02d:%02d:%s%.2fZ,", year, mon, day, hr, minute, sec < 10.0 ? "0" : "", sec);
          long stopTime = (hr * 60 * 60) + (minute * 60) + sec;

          long duration = stopTime - startTime;
          if (duration < 0) // Does the pass cross midnight?
            duration += 24 * 60 * 60;

          Serial.printf("%02d:%02d:%02d,", duration / (60 * 60), (duration / 60) % 60, duration % 60);
          
          invjday(overpass.jdmax ,timezone ,true , year, mon, day, hr, minute, sec);
          Serial.printf("%s%.3f,", overpass.maxelevation < 10.0 ? "0" : "", overpass.maxelevation);

          Serial.println(sat.satName);
...
void setup()
{
  ...
  while (keepGoing)
  {
    ...
    else
    {
      ...
      Predict(5); //Calculates the next five overpasses
